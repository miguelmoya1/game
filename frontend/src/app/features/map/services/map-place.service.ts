...
export class MapPlaceService {
  ...
  public addPlaces(places: PlaceList[]) {
    ...
    places.forEach((place) => {
      ...
      if (!existingMarker) {
        ...
        existingMarker.getElement()?.setAttribute('style', `--marker-fill-color: ${this.#getMarkerColor(place)}`);
  ...
  #getMarkerColor(place: PlaceList) {
    return place.permissions.canBeClaimed ? 'var(--color-primary)' : 'var(--color-warning)';
  }

  #createElementMarker(place: PlaceList): HTMLElement {
    const element = document.createElement('div');
    const initialColor = place.permissions.canBeClaimed ? 'var(--color-primary)' : 'var(--color-warning)';
