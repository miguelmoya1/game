...
import { Create, PlayerItemCollectionLogRepository } from '../contracts/player-item-collection-log.repository.contract';
import { playerItemCollectionLogToEntity } from '../mappers/player-item-collection-log.mapper';

@Injectable()
export class PlayerItemCollectionLogRepositoryImpl implements PlayerItemCollectionLogRepository {
  ...
  async add(create: Create): Promise<PlayerItemCollectionLogEntity> {
    const playerItemCollectionLog = await this.databaseService.playerItemCollectionLog.create({
      data: {
  ...
  async getForPlaces(
    ...
    const playerItemCollectionLogs = await this.databaseService.playerItemCollectionLog.findMany({
      where: {
        placeId: {
          in: placeIds,
        },
