...
export class LoginWithEmailHandler implements ICommandHandler<LoginWithEmailCommand> {
  ...
  async execute(command: LoginWithEmailCommand) {
    ...
    const isPasswordValid = await this._encryptionService.compare(password, account.password!);

    if (!isPasswordValid) {
      throw new HttpException(ErrorCodes.USER_OR_PASSWORD_INVALID, HttpStatus.UNAUTHORIZED);
    }
