...
export class GenerateDungeonsHandler
  implements ICommandHandler<GenerateDungeonsCommand>
{
  ...
  async execute() {
    ...
    const randomTypes = Object.values(DungeonType).sort(
      () => Math.random() - 0.5,
    );
    ...
    for (const place of placeIdsToGenerate) {
      const rank =
        randomRanked[Math.floor(Math.random() * randomRanked.length)];
      ...
      const itemsSameCategory = items
        ...
        .filter((item) =>
          item.spawnCategories?.some((cat) => categories.includes(cat)),
        );

      const rewardsCount = 1 + Math.floor(Math.random() * 3); // 1, 2 or 3
      const shuffled = itemsSameCategory.sort(() => Math.random() - 0.5);
      const rewards = shuffled
        .slice(0, Math.min(rewardsCount, shuffled.length))
        .map((item) => ({
